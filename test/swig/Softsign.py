...
class SoftsignTest(unittest.TestCase):
    ...
    def test_Softsign1D (self):
        npr = softsign_np(self.np_a)
        dcr = dc.softsign(self.dc_a)
        np.testing.assert_allclose(npr, np.array(dcr.data()).astype(np.float32),
                rtol=1e-3, atol=1e-3)
    
    def test_Softsign2D (self):
        np_a = np.reshape(self.np_a, (6,4))
        dc_a = dc.reshape(self.dc_a, (6,4))
        npr = softsign_np(np_a)
        dcr = dc.softsign(dc_a)
        np.testing.assert_allclose(npr.flatten(), np.array(dcr.data()).astype(np.float32),
                rtol=1e-3, atol=1e-3)

    def test_Softsign3D (self):
