...
class ConvTest(unittest.TestCase):
    def setUp(self):
        ...
        self.onnx_npr   = np.array([12.,21.,27., 33., 24., 33., 54., 63., 72., 51., 63.,99.,108.,117., 81., 93.,144.,153.,162.,111., 72.,111.,117.,123., 84.])
...
#        np.testing.assert_allclose(npr, np.array(dcr.data()).astype(np.float32),rtol=1e-3, atol=1e-3)
    ...
    def test_onnx_conv_example (self):     
        dcr            = dc.conv(self.onnx_dc_X, self.onnx_dc_W,  self.dc_B)
        np.testing.assert_allclose(self.onnx_npr.astype(np.float32), np.array(dcr.data()).astype(np.float32),rtol=1e-3, atol=1e-3)
