...
namespace dnnc {
template <typename To, typename Ti> class ReduceSum : public baseOperator<To, Ti, Ti> {
...
public:
  ...
  tensor<To> compute(tensor<Ti> input) {
    ...
    //new shape of the tensor will be composed of the original tensor shape with values in _axis removed
    for (int axis : _axis) {
    ...
    if (rank == 1) {
      for (size_t l0 = 0; l0 < shape[0]; l0++) {
        ...
        result(k) += static_cast<To>(input(l0));
      }
    }
    if (rank == 2) {
      for (size_t l0 = 0; l0 < shape[0]; l0++) {
