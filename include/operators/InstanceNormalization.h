...
/*! According to this Mean of the elements in channel is prerequisite for Variance calculation.
    A little bit of maths will tell you that mean is not required for Variance they can be calculated simultaneously.*/

/*! \f$ var(X) = \frac{1}{n}\sum_{i=1}^{n} (x_{i}-\mu)^{2} = \frac{1}{n}\sum_{i=1}^{n} (x_{i}^{2}-2\mu x_{i}+\mu^{2})= \frac{1}{n}\sum_{i=1}^{n}x_{i}^{2}-\frac{2\mu}{n}\sum_{i=1}^{n} x_{i}+ \mu\frac{n}{n} = \frac{1}{n}\sum_{i=1}^{n}x_{i}^{2} - \mu^2 \f$ */
...
public:
  ...
  tensor<T> compute(tensor<T> &input /*!< [float,double]: ND tensor of shape ( NxCxD1xD2…Dk ).*/, tensor<T> &scale /*!<  1D vector of dimension C.*/, tensor<T> &B /*!< : 1D vector of dimension C.*/)
  {
    if (!compare())
