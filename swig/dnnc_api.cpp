...
tensor<float> dequantize_linear(tensor<int> &a, tensor<float> &b,
                                tensor<int> &c) {
...
tensor<double> gemm(tensor<double> &a, tensor<double> &b, tensor<double> &c,
...
tensor<double> hardsigmoid(tensor<double> &a, float alpha = 0.2,
                           float beta = 0.5) {
...
tensor<float> instancenormalization(tensor<float> &a, tensor<float> &scale,
                                    tensor<float> &B, float epsilon = 1e-5) {
  InstanceNormalization<float> op("localOpName", epsilon);
  return op.compute(a, scale, B);
}

tensor<double> instancenormalization(tensor<double> &a, tensor<double> &scale,
                                     tensor<double> &B, float epsilon = 1e-5) {
...
tensor<bool> isinf(tensor<double> &a, int detect_positive = 1,
                   int detect_negative = 1) {
...
tensor<float> lrn(tensor<float> &a, int size, float alpha = 0.0001,
                  float beta = 0.75, float bias = 1.0) {
  LRN<float> op(size, "localOpName", alpha, beta, bias);
  return op.compute(a);
}

tensor<double> lrn(tensor<double> &a, int size, float alpha = 0.0001,
                   float beta = 0.75, float bias = 1.0) {
...
tensor<int> matmulinteger(tensor<int> &a, tensor<int> &b, tensor<int> &c,
                          tensor<int> &d) {
...
tensor<double> slice(tensor<double> &a, tensor<size_t> &start,
                     tensor<size_t> &end, tensor<int> axes = NULL_TENSOR<int>,
                     tensor<size_t> steps = NULL_TENSOR<size_t>) {
  Slice<double> op;
  return op.compute(a, start, end, axes, steps);
}

tensor<float> slice(tensor<float> &a, tensor<size_t> &start,
                    tensor<size_t> &end, tensor<int> axes = NULL_TENSOR<int>,
                    tensor<size_t> steps = NULL_TENSOR<size_t>) {
  Slice<float> op;
  return op.compute(a, start, end, axes, steps);
...
tensor<int> slice(tensor<int> &a, tensor<size_t> &start, tensor<size_t> &end,
...
tensor<bool> slice(tensor<bool> &a, tensor<size_t> &start, tensor<size_t> &end,
                   tensor<int> axes = NULL_TENSOR<int>,
                   tensor<size_t> steps = NULL_TENSOR<size_t>) {
  Slice<bool> op;
  return op.compute(a, start, end, axes, steps);
